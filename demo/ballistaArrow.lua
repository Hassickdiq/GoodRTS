function BALLISTA_ARROWSpawn(self, globaltime, dt)
...
function BALLISTA_ARROWTouch(self, touchEntity, dt)
	if self[1] ~= touchEntity.id and self.ownerID ~= touchEntity.ownerID then
...
function BALLISTA_ARROWThink(self, gt, dt)
	local rads = math.rad(self.transform_angles)
	local dirx = math.cos(rads)
	local diry = math.sin(rads)
	
	self.transform_origin_x = self.transform_origin_x + dirx * 250 * dt
	self.transform_origin_y = self.transform_origin_y + diry * 250 * dt
